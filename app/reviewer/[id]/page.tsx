...
const STATUS_OPTIONS = [
  { value: "SUBMITTED",    label: "Menunggu Review",  cls: "text-blue-400 border-blue-700 bg-blue-900/30" },
  { value: "UNDER_REVIEW", label: "Sedang Ditinjau",  cls: "text-amber-400 border-amber-700 bg-amber-900/30" },
...
export default function ReviewerPage() {
  ...
  // Admin-only: status & reviewer assignment
  ...
  // Load list of reviewers for assignment (admin only)
  useEffect(() => {
    if (!isAdmin) return
  ...
  if (error || !article) return (
    <div className="bg-ocean-950 min-h-screen flex items-center justify-center">
      <div className="text-center space-y-3">
        ...
        <a href="/dashboard/reviewer" className="text-ocean-400 hover:text-gold-400 text-sm transition-colors">
          ← Kembali ke Reviewer
        </a>
      </div>
    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <a href="/dashboard/reviewer"
              className="p-1.5 rounded-lg text-ocean-500 hover:text-white hover:bg-ocean-800 transition-colors flex-shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-ocean-500 text-[10px] font-mono">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-ocean-900/40 border border-ocean-800 rounded-xl p-5">
              ...
              <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={k}>
                    <dt className="text-ocean-600 text-[10px] mb-0.5">{k}</dt>
          ...
          <div className="space-y-4">
            <div className="sticky top-20 space-y-4">
              ...
              {isAdmin && (
                <div className="bg-ocean-900/40 border border-ocean-700/60 rounded-xl p-4 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <p className="text-[10px] text-ocean-500 uppercase tracking-widest font-mono">Status</p>
                    <div className="grid grid-cols-1 gap-1">
                      {STATUS_OPTIONS.map((opt) => (
                        <label key={opt.value}
                          className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer
                                      transition-all text-xs font-medium
                                      ${status === opt.value ? opt.cls : "border-ocean-800 text-ocean-500 hover:border-ocean-700 hover:text-ocean-300"}`}>
                  ...
                  {/* Assign Reviewer */}
                  <div className="space-y-1.5">
                    <p className="text-[10px] text-ocean-500 uppercase tracking-widest font-mono">Tugaskan Reviewer</p>
                    <select value={assignedReviewerId} onChange={e => setAssignedReviewerId(e.target.value)}
                      className="w-full px-3 py-2 bg-ocean-950 border border-ocean-700 rounded-lg text-sm
                                 text-white focus:outline-none focus:border-ocean-500">
                  ...
                  {/* Catatan */}
                  <div className="space-y-1.5">
                    <p className="text-[10px] text-ocean-500 uppercase tracking-widest font-mono">Catatan untuk Penulis</p>
                    <textarea value={reviewerNote} onChange={e => setReviewerNote(e.target.value)} rows={4}
                      ...
                      className="w-full px-3 py-2 bg-ocean-950 border border-ocean-700 rounded-lg text-sm
                                 text-white placeholder:text-ocean-700 focus:outline-none focus:border-ocean-500 resize-none" />
                  </div>
              ...
              {/* REVIEWER: catatan rekomendasi saja */}
              {!isAdmin && isReviewer && (
                <div className="bg-ocean-900/40 border border-ocean-800 rounded-xl p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-white font-semibold text-sm">Catatan Review</p>
                  </div>
                  <p className="text-ocean-500 text-xs">Catatan Anda akan diteruskan ke editor untuk pengambilan keputusan.</p>
                  ...
                  <button onClick={async () => {
                    ...
                    className="w-full py-2 rounded-lg bg-ocean-700 hover:bg-ocean-600 text-white
              ...
              {isReviewer && pendingComment && (
                <div className="bg-gold-900/20 border border-gold-700/50 rounded-xl p-4 space-y-3">
                  ...
                  <textarea value={commentInput} onChange={e => setCommentInput(e.target.value)}
                    ...
                    className="w-full px-3 py-2 bg-ocean-950 border border-ocean-700 rounded-lg text-sm
                               text-white placeholder:text-ocean-600 focus:outline-none focus:border-ocean-500 resize-none" />
              ...
              <div className="space-y-2">
                <p className="text-ocean-500 text-xs font-medium flex items-center gap-2">
                  Komentar
                  {comments.length > 0 && (
                    <span className="text-white font-mono">({comments.length})</span>
                  )}
                </p>
                {comments.length === 0 ? (
                  <p className="text-ocean-700 text-xs text-center py-4 bg-ocean-900/20 rounded-lg border border-dashed border-ocean-800">
                    {isReviewer ? "Blok teks lalu klik Tambah Komentar" : "Belum ada komentar"}
                  </p>
                ) : (
                  [...comments].reverse().map((c) => (
                    <div key={c.id} className="bg-ocean-900/40 border border-ocean-800 rounded-xl p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        {(isAdmin || currentUser?.name === c.reviewerName) && (
                          <button onClick={() => deleteComment(c.id)}
                            className="text-ocean-700 hover:text-red-400 text-xs transition-colors">✕</button>
                        )}
                      </div>
                      <blockquote className="border-l-2 border-ocean-700 pl-2 text-[11px] text-ocean-500 italic line-clamp-2">
                        "{c.selectionText}"
                      </blockquote>
...
function FileChip({ label, url }: { label: string; url: string }) {
  return (
    <a href={url} target="_blank" rel="noopener noreferrer"
       className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-ocean-900
                  border border-ocean-700 hover:border-ocean-500 text-xs text-ocean-300
