...
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
  // Konversi id dari string (URL) ke number (database)
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-ocean-900 border border-ocean-800 rounded-2xl overflow-hidden shadow-xl shadow-ocean-950/50">
          ...
          <div className="px-6 sm:px-10 py-8 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-ocean-950/50 rounded-xl p-4 border border-ocean-800">
                ...
                <ul className="space-y-1.5">
                  {/* authors sekarang array objek, bukan array string */}
                  {article!.authors.map((author) => (
                    <li key={author.id} className="flex items-center gap-2 text-sm text-ocean-200">
                      <svg className="w-3.5 h-3.5 text-ocean-600 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z" />
                      </svg>
                      {author.name}
                      {author.affiliation && (
                        <span className="text-ocean-500 text-xs">— {author.affiliation}</span>
                      )}
                    </li>
                  ))}
                </ul>
            ...
            {/* Kata kunci — hanya tampil jika ada */}
