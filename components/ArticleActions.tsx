...
// Komponen CLIENT untuk tombol-tombol interaktif di halaman detail artikel.
// Dipisah dari page.tsx (Server Component) karena menggunakan:
//   - onClick (event handler)
//   - navigator.clipboard (Web API browser)
//   - alert()
// Dengan pemisahan ini, Server Component tetap efisien (di-render di server)
// sementara interaktivitas hanya dikirim ke browser untuk komponen ini saja.
// ============================================================
...
export default function ArticleActions({ article }: ArticleActionsProps) {
  // Fungsi untuk menyalin kutipan format APA ke clipboard
  const handleCopyCitation = () => {
    const year = new Date(article.publishedDate).getFullYear();
    const citation =
      `${article.authors.join(", ")} (${year}). ` +
      `${article.title}. ` +
      `CONSERVE Journal of Community Services, ${article.volume}(${article.number}), ` +
    ...
    if (navigator.clipboard) {
      ...
      // Fallback untuk browser lama yang tidak mendukung Clipboard API
      alert("Browser Anda tidak mendukung salin otomatis. Silakan salin secara manual.");
    }
  };

  // Fungsi untuk menyalin DOI ke clipboard
  const handleCopyDOI = () => {
    if (navigator.clipboard) {
    ...
    <div className="space-y-4">
      ...
      <p className="text-[11px] text-ocean-600 text-center pt-1">
        * Tautan unduhan PDF ini bersifat simulasi untuk keperluan demonstrasi.
        Dalam implementasi nyata, tautan akan mengarah ke file PDF yang sesungguhnya.
      </p>
    </div>
  );
}
