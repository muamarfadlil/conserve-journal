  ...
  return (
    <div className="relative flex-shrink-0" ref={panelRef}>
      <button
        ...
        className="relative p-2 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)]
      ...
      <div className={`absolute right-0 top-full mt-2 w-80 z-50
                       bg-[var(--bg-surface)] backdrop-blur-sm border border-[var(--border-default)]
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-default)]">
          <div className="flex items-center gap-2">
            <h3 className="text-[var(--text-primary)] text-sm font-semibold">Notifikasi</h3>
            {unread > 0 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-red-100 dark:bg-red-900/60
                               border border-red-300 dark:border-red-700 text-red-600 dark:text-red-300 font-mono">
                {unread} baru
              </span>
            )}
          </div>
        ...
        <div className="max-h-80 overflow-y-auto">
          {loading ? (
            ...
            <div className="py-10 text-center">
              <svg className="w-8 h-8 text-[var(--text-muted)] mx-auto mb-2 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
                      d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
              <p className="text-[var(--text-muted)] text-xs">Belum ada notifikasi</p>
            </div>
          ) : (
            notifs.map(n => {
              ...
              return (
                <Wrapper
                  ...
                  className={`flex items-start gap-3 px-4 py-3 border-b border-[var(--border-default)]
                              hover:bg-[var(--bg-surface-alt)] transition-colors cursor-pointer
                              ${!n.isRead ? "bg-ocean-50 dark:bg-ocean-800/20" : ""}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-[var(--text-muted)] opacity-60 mt-1">{timeAgo(n.createdAt)}</p>
                  </div>
                </Wrapper>
              )
            })
